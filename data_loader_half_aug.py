...
class DataLoader(object):
    ...
    def get_aug_support(self,support_triples,rel,max_num):
        aug = []
        keylist = list(self.task_aug[rel].keys())
        for key in keylist:
            aug.append(self.task_aug[rel][key])

        diminished = [token for st in aug for token in st]
        num = min(max_num, len(diminished))

        top_num = len(self.task_aug[rel][keylist[0]])
        if top_num < num:
            for j in range(top_num):
                support_triples.append([diminished[j][0],rel,diminished[j][2]])
            first = diminished[top_num:]
            for j in range(num-top_num):
                item = random.choice(first)
        ...
        #     top_rel = min(len(self.task_aug[rel][str(1)]),max_num)
